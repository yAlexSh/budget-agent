...
START_TEXT = (
    ...
    "осталось, отвечаю на вопросы о ваших тратах и накоплениях, а если "
    "спросите про общие принципы обращения с деньгами — отвечу с опорой на "
    "источники, а не просто от себя.\n\n"
    "Мне необязательно писать команды: расскажите о трате или задайте "
    "вопрос обычным текстом, как сказали бы вслух.\n\n"
    "Для начала — какой месячный лимит трат ставим? Ответьте числом в "
    "рублях.")
...
HELP_TEXT = (
    ...
    "/help — эта справка\n\n"
    "Траты записывайте обычным текстом, без специального формата: "
    "«потратил 850 в Пятёрочке» или «300 рублей такси» — сумму и место "
    "я разберу сам.\n\n"
    "Вопросы тоже можно задавать свободно, вот несколько примеров:\n"
    "- про свои цифры: «хватит ли до зарплаты»\n"
    "- про принципы: «сначала гасить кредит или копить подушку»\n"
    "- про планы: «отпуск обойдётся в 280 тысяч — потянем?»\n"
    "- про рынок: «какая сейчас ключевая ставка»\n\n"
    "Приватность устроена просто: в личном чате вам видны и ваши траты, и "
    "общие семейные, а в общем чате — только общие. Это сделано намеренно, "
    "чтобы личные покупки не всплывали при всех.\n\n"
    "Чего я не делаю: не оплачиваю покупки и не перевожу деньги, не даю "
    "индивидуальных инвестиционных рекомендаций и работаю только с тем, "
    "что вы сами внесли.\n\n"
    "Команды отвечают сразу. А ответ на содержательный вопрос занимает "
    "несколько секунд — в этот момент я смотрю в данные и справочники, "
    "это не зависание.")
